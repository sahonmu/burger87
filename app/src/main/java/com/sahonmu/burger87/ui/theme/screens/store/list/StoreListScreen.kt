...
fun StoreListScreen(
    ...
    Column(
        ...
        if (storeListUiState.loadState == LoadState.LOADING) {
            ProgressDialog()
        } else if (storeListUiState.loadState == LoadState.EMPTY) {
            EmptyBox(
                emptyMessage = "상점목록이 없습니다."
            )
        } else if (storeListUiState.loadState == LoadState.FINISHED) {
